...
err_t eth0_init(struct netif *netif) {
	...
	alt_avalon_sgdma_register_callback(eth0_dma_rx_dev, (alt_avalon_sgdma_callback) eth0_rx_interrupt, 0x00000014, NULL );
	do {
...
void eth0_rx_interrupt(void *context) {
	...
	eth0_rx_len = eth0_rx_descriptor.actual_bytes_transferred;
}
...
err_t eth1_init(struct netif *netif) {
	...
	alt_avalon_sgdma_register_callback(eth1_dma_rx_dev, (alt_avalon_sgdma_callback) eth1_rx_interrupt, 0x00000014, NULL );
	do {
...
void eth1_rx_interrupt(void *context) {
	...
	eth1_rx_len = eth1_rx_descriptor.actual_bytes_transferred;
}
